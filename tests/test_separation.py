...
class FakeFactory:
    ...
    @classmethod
    def create(cls, model: Model) -> FakeAlgorithm:
...
def db_session(monkeypatch):
    ...
    db = TestingSession()
    monkeypatch.setattr(separation_service, "HEART_OUTPUT_DIR", runtime_dir / "heart")
    monkeypatch.setattr(separation_service, "LUNG_OUTPUT_DIR", runtime_dir / "lung")
    FakeFactory.algorithm = FakeAlgorithm()
    FakeFactory.model_ids = []
    monkeypatch.setattr(separation_service, "SeparationAlgorithmFactory", FakeFactory)
...
def test_separation_uses_active_model_by_default(db_session) -> None:
    ...
    assert FakeFactory.model_ids == [model.model_id]
