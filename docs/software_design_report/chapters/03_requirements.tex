...
class StorageService:
    # SRP: this class changes when storage rules change.
...
class SeparationEngine:
    # OCP: closed to changes in the engine workflow.
...
class NeoSSNetStrategy:
    # OCP: open for extension through a concrete strategy.
...
The open part is the set of concrete \texttt{SeparationAlgorithm} implementations. The closed part is \texttt{SeparationEngine}; it can keep the same call structure while a future real algorithm is added through a new strategy class and model-strategy resolver registration.
...
def run_engine(algorithm: SeparationAlgorithm, paths):
    # LSP: any valid SeparationAlgorithm can be used here.
...
# LSP: NeoSSNetStrategy is substitutable for SeparationAlgorithm.
result = run_engine(NeoSSNetStrategy(), paths)
\end{lstlisting}
...
class AudioValidator(Protocol):
    # ISP: upload code only needs validation behavior.
    async def validate(self, upload_file) -> None:
        ...


class SeparationAlgorithm(Protocol):
    # ISP: the engine only needs separation behavior.
    def separate(self, input_wav_path, model_path, model_config_path,
                 heart_output_path, lung_output_path):
        ...
\end{lstlisting}
...
class SeparationEngine:
    def __init__(self, algorithm: SeparationAlgorithm):
        # DIP: depend on the abstraction, not NeoSSNetStrategy.
        self.algorithm = algorithm
