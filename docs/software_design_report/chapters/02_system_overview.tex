...
Design pattern roles are shown through stereotypes and the diagram legend rather than by duplicating class boxes. For example, \texttt{SeparationService} is marked as the Facade, Strategy Context, and Factory Client; \texttt{SeparationAlgorithm} is marked as the Strategy/Product abstraction; \texttt{NeoSSNetStrategy} is marked as the Concrete Strategy/Concrete Product; and \texttt{SeparationAlgorithmFactory} is marked as the Creator. This keeps the official class diagram correct while still making the selected design patterns visible.
