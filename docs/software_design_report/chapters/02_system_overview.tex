...
\begin{figure}[H]
  ...
  \caption{Pattern-Grouped Overall UML Class Diagram}
  \label{fig:overall-class-diagram}
\end{figure}
\end{landscape}

Figure~\ref{fig:overall-class-diagram} presents the main classes and interfaces in the cardiopulmonary sound separation system using design-pattern groupings. Instead of grouping the classes by technical layers such as routers, services, or database models, the diagram groups them by the Facade, Strategy, and Factory Method patterns. This makes the pattern roles easier to explain while still using UML class-style boxes, selected methods, stereotypes, and relationships.

The same class may appear in more than one pattern container because it can play a different role in each design pattern. For example, \texttt{SeparationService} is the Facade in the Facade Pattern, the Context in the Strategy Pattern, and the Client in the Factory Method Pattern. \texttt{NeoSSNetStrategy} implements the \texttt{SeparationAlgorithm} interface, while \texttt{SeparationAlgorithmFactory} creates the selected algorithm object for the separation workflow.

\clearpage
\section{Design Pattern Location Overview}

The following diagram highlights the same pattern locations in a simpler presentation-oriented view. It is grouped by design pattern rather than by technical category so the Facade, Strategy, and Factory Method roles can be explained quickly.
