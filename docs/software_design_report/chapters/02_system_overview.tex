...
Design pattern roles are separated so that one main class is not overloaded with multiple pattern labels. \texttt{SeparationService} is shown only as the Facade used by \texttt{SeparationRouter}. \texttt{SeparationEngine} is shown as the Strategy Context that depends on \texttt{SeparationAlgorithm}, and \texttt{NeoSSNetStrategy} is the current Concrete Strategy. \texttt{UploadRouter} is the Factory Method Client, \texttt{AudioValidatorFactory} is the Creator, \texttt{WavAudioValidatorFactory} is the ConcreteCreator, \texttt{AudioValidator} is the Product, and \texttt{WavAudioValidator} is the ConcreteProduct. \texttt{ModelStrategyResolver} appears only as a supporting model-selection helper, not as the selected Factory Method pattern.
