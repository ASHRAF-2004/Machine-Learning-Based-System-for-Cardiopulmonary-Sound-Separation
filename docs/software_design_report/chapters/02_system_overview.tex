...
Figure~\ref{fig:overall-class-diagram} presents the main classes and interfaces in the cardiopulmonary sound separation system. Each class or interface appears only once, and the diagram shows the API router modules, facade services, strategy classes, separation algorithm factory, upload validation support classes, and SQLite data model classes. The diagram uses UML class-style boxes, selected attributes or methods, associations, dependencies, and interface implementation arrows to describe the whole system structure.

Design pattern roles are separated so that one main class is not overloaded with multiple pattern labels. \texttt{SeparationService} is shown as the Facade used by \texttt{SeparationRouter}. \texttt{SeparationEngine} is shown as the Strategy Context that depends on \texttt{SeparationAlgorithm}, and \texttt{NeoSSNetStrategy} is the current Concrete Strategy. \texttt{SeparationAlgorithmFactory} is the Factory Method Creator that creates a \texttt{SeparationAlgorithm} product from the selected SQLite \texttt{Model} record. \texttt{NeoSSNetStrategy} is the current ConcreteProduct. \texttt{ModelStrategyResolver} remains only as a compatibility name for older code and tests.
