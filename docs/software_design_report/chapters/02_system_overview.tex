...
The following diagrams describe the system from different software design viewpoints. The use case diagram presents the system functions from the user's perspective, the architecture diagram shows the main runtime layers, the component level diagram shows how modules interact, the class diagram shows the planned software components, the entity relationship diagram shows the database structure, and the object diagram gives an example of one completed separation job.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth,height=0.72\textheight,keepaspectratio]{architecture_diagram.png}
  \caption{Software Architecture Diagram}
  \label{fig:architecture-diagram}
\end{figure}

Figure~\ref{fig:architecture-diagram} shows the overall architecture of the prototype. The user interacts with a browser-based web interface, which sends RESTful HTTP requests to the FastAPI application. The FastAPI application is separated into route handling, business services, and the machine learning boundary. SQLite stores structured metadata such as uploads, jobs, result paths, metrics, and logs, while local storage folders keep uploaded WAV files, separated heart and lung outputs, model files, and runtime logs.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1.22\textwidth,height=0.78\textheight,keepaspectratio]{component_level_diagram.png}
  \caption{Component Level Diagram}
  \label{fig:component-level-diagram}
\end{figure}
\end{landscape}

Figure~\ref{fig:component-level-diagram} shows the main software components and their responsibilities. The UI layer handles upload, preview, download, and history screens. The FastAPI router layer receives requests for upload, separation, model listing, result retrieval, download, and history. The business service layer coordinates validation, separation workflow, model lookup, file storage, and result persistence. The ML layer runs separation through \texttt{SeparationEngine}, \texttt{SeparationAlgorithm}, and \texttt{NeoSSNetStrategy}. SQLite stores processing records, while local storage keeps the actual audio and model files.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1.22\textwidth,height=0.78\textheight,keepaspectratio]{overall_class_diagram.png}
  \caption{Overall System UML Class Diagram}
  \label{fig:overall-class-diagram}
\end{figure}
\end{landscape}

Figure~\ref{fig:overall-class-diagram} presents the main classes and interfaces in the cardiopulmonary sound separation system. Each class or interface appears only once, and the diagram shows the API router modules, facade services, strategy classes, upload validation factory classes, and SQLite data model classes. The diagram uses UML class-style boxes, selected attributes or methods, associations, dependencies, and interface implementation arrows to describe the whole system structure.

Design pattern roles are separated so that one main class is not overloaded with multiple pattern labels. \texttt{SeparationService} is shown as the Facade used by \texttt{SeparationRouter}. \texttt{SeparationEngine} is shown as the Strategy Context that depends on \texttt{SeparationAlgorithm}, and \texttt{NeoSSNetStrategy} is the current Concrete Strategy. \texttt{UploadRouter} is shown as the Factory Method client, \texttt{AudioValidatorFactory} is the Creator, \texttt{WavAudioValidatorFactory} is the ConcreteCreator, \texttt{AudioValidator} is the Product interface, and \texttt{WavAudioValidator} is the ConcreteProduct.
