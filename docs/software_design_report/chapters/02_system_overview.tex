...
Figure~\ref{fig:overall-class-diagram} presents the main classes and interfaces in the cardiopulmonary sound separation system. The diagram shows the FastAPI routers, service classes, machine learning abstraction, concrete NeoSSNet strategy, factory component, and SQLite data models. It is a UML class diagram because it represents classes, selected fields or methods, and relationships between those classes.

The diagram also shows the key design relationships. \texttt{SeparationRouter} delegates separation requests to \texttt{SeparationService}, while \texttt{UploadRouter} uses \texttt{StorageService} to save uploaded WAV files. The separation service selects a model, prepares storage paths, asks the factory to create an algorithm object, and creates or updates job and result records. \texttt{NeoSSNetStrategy} implements the \texttt{SeparationAlgorithm} interface, which keeps the class design extensible for future implemented separation algorithms.

\clearpage
\section{Design Pattern Location Overview}

The following diagram highlights where the selected design patterns are located in the overall class design. It is not the official UML class diagram; instead, it is a presentation-oriented overview that groups the most important pattern participants so the Facade, Strategy, and Factory Method roles can be explained quickly.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth,height=0.60\textheight,keepaspectratio]{pattern_location_overview.png}
  \caption{Design Pattern Location Overview}
  \label{fig:pattern-location-overview}
\end{figure}

Figure~\ref{fig:pattern-location-overview} shows that \texttt{SeparationService} acts as the Facade. It also shows that the factory creates the selected algorithm object, while \texttt{SeparationAlgorithm} with \texttt{NeoSSNetStrategy} forms the Strategy structure. This diagram is kept separate from the class diagram because it explains design pattern placement rather than complete class structure.
